...
function Snake({ playerId, color, isLocal }: { playerId: string, color: string, isLocal: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    // Update colors from global state for instant feedback
    if (player.color) {
      (headRef.current.material as THREE.MeshStandardMaterial).color.set(player.color);
      (bodyRef.current.material as THREE.MeshStandardMaterial).color.set(player.color);
    }
      ...
      for (const orbId in gs.orbs) {
        ...
        if (dx * dx + dy * dy < 4) {
          ...
          if (gs.players[playerId]) {
            gs.players[playerId].color = orb.color;
          }
